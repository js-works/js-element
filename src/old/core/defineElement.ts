...
type ValidPropsConfig<
  P extends Props,
  PC extends PropsConfig<P>
> = keyof PC extends keyof P
  ? keyof P extends keyof PC
    ? 1 extends {
        [K in keyof PC]: {} extends Omit<PC[K], keyof PropConfig<any>> ? 0 : 1
      }[keyof PC]
      ? never
      : PC
    : never
...
type ConfigStatefulWithMethods<
  P extends Props,
  M extends Methods,
  PC extends PropsConfig<P>
> = {
...
function defineElement<
  ...
  M extends Methods,
  PC extends PropsConfig<P>
>(config: ConfigStatefulWithMethods<P, M, PC>): void
...
function defineElement(a: any, b?: any, c?: any) {
  // TODO
  ...
  class litElementClass extends LitElement {
    render() {
      return html`<div>Juhu</div>`
    }
  }
...
function generateCustomElementClass(tagName: any, options: any, main: any) {
  // TODO
  const propNames = options.props ? keysOf(options.props) : [],
  ...
  if (options.methods && options.methods.length > 0) {
    options.methods.forEach((methodName: any) => {
      // TODO
      ;(CustomElement as any).prototype[methodName] = function () {
        // TODO
  ...
  propNames
    .filter((it) => !isEventPropName(it))
    .forEach((propName) => {
      const propConfig = options.props[propName],
...
class BaseElement extends HTMLElement {
  constructor(statics: any) {
    // TODO
    super()
    const self: any = this // TODO
    self._statics = statics
    self._props = statics.defaultProps
      ? Object.assign({}, statics.defaultProps)
      : {}
    ...
    statics.eventPropNames.forEach((eventPropName: string) => {
      ...
      Object.defineProperty(self._props, eventPropName, {
        get() {
          const listenerSet =
            self._listenersByEventName && self._listenersByEventName[eventName]
  ...
  getAttribute(this: any, attrName: string) {
    // TODO
  ...
  _refresh(this: any) {
    ...
    this._mounted &&
      ...
      this._beforeUpdateNotifier.notify()

    if (
      this._mounted &&
      this._runOnceBeforeUpdateTasks &&
      this._runOnceBeforeUpdateTasks.length
    ) {
  ...
  _update(this: any, runOnceBeforeUpdate: () => void) {
    // TODO
    runOnceBeforeUpdate &&
      (this._runOnceBeforeUpdateTasks ||
        (this._runOnceBeforeUpdateTasks = [])) &&
  ...
  _afterMount(this: any, callback: () => void) {
    // TODO
  ...
  _beforeUpdate(this: any, callback: () => void) {
    // TODO
  ...
  _afterUpdate(this: any, callback: () => void) {
    // TODO
  ...
  _beforeUnmount(this: any, callback: () => void) {
    // TODO
...
function checkCustomElementConfig(name: string, options: any) {
  // TODO
